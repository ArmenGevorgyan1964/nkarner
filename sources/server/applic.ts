...
function getCalcHtml(req:Http.IncomingMessage, resp:Http.ServerResponse, data){
    ...
    if(data && data.txtSecondNo && data.txtSecondNo){
        let sum:number = parseInt(data.txtFirstNo)  + parseInt(data.txtSecondNo);
        sb.appendLine("         <tr>");
...
function getCalcForm(req:Http.IncomingMessage, resp:Http.ServerResponse, data:object){
    resp.writeHead(200, {'Content-Type':'text/html'} );
    getCalcHtml(req, resp, data);

}
Http.createServer(function(req:Http.IncomingMessage, resp:Http.ServerResponse ){
       console.log(req.url);
        switch(req.method){
            case "GET":
               if(req.url === "/"){
            ...
            case 'POST':
                ...
                if(req.url === "/calc"){
                    ...
                    req.on('end', function(data){
                        ...
                        var formData:object = qs.parse(requestBody);
